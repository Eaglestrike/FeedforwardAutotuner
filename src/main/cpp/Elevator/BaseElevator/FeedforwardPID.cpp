...
double FeedforwardPID::periodic(Poses::Pose1D current_pose)
{
    ...
    double feedforward_voltage = calculateFeedforwardVoltage(expected_pose.vel, expected_pose.acc);
    ...
    if(shuffleboard){
        ...
        frc::SmartDashboard::PutNumber("expected ev velocity", expected_pose.vel);
        frc::SmartDashboard::PutNumber("expected ev position", expected_pose.pos);
        frc::SmartDashboard::PutNumber("expected acceleration", expected_pose.acc);

        frc::SmartDashboard::PutNumber("position error", expected_pose.pos - current_pose.pos);
        frc::SmartDashboard::PutNumber("velocity error", expected_pose.vel - current_pose.vel);
        frc::SmartDashboard::PutNumber("acceleration error", expected_pose.acc - current_pose.acc);
...
{
    accum_ += dt * (expected.pos - current.pos);
    double posErr = expected.pos - current.pos;
    if (std::abs(posErr) < ElevatorConstants::POSITION_ERROR_TOLERANCE){
        posErr = 0.0;
    }
    return kp * posErr + ki * accum_ + kd * (expected.vel - current.vel);
}
...
Poses::Pose1D FeedforwardPID::getExpectedPose(double time)
{
    ...
    if (time < 0){
        ...
        pose.acc = reversed_coefficient * max_acceleration;
        pose.vel = reversed_coefficient * max_acceleration * time;
        pose.pos = reversed_coefficient * 0.5 * pose.vel * time;
    }
    // if in the velocity phase
        ...
        pose.acc = 0.0;
        pose.vel = reversed_coefficient * max_velocity;
        // adds phase 1 to however much of phase 2 has been gone through
        pose.pos = reversed_coefficient * 0.5 * max_velocity * acceleration_time + max_velocity * reversed_coefficient * (time - acceleration_time);
    }
    // if in the deceleration phase
        ...
        pose.acc = -1.0 * reversed_coefficient * max_acceleration;
        pose.vel = reversed_coefficient * max_vel - (reversed_coefficient * max_acceleration * (time_in_triangle));

        // trapezoidal area
        double third_phase_distance = reversed_coefficient * (max_velocity + pose.vel) / 2.0 * time_in_triangle;

        pose.pos = first_phase_distance + second_phase_distance + third_phase_distance;
    }

    else{
        pose.pos = reversed_coefficient * total_distance_;
        pose.vel = 0.0;
        pose.acc = 0.0;
    }

    pose.pos += startpoint_;
...
bool FeedforwardPID::atSetpoint(Poses::Pose1D current_pose, double xTol, double vTol){
    double xError = current_pose.pos - setpoint_;
    double vError = current_pose.vel;
